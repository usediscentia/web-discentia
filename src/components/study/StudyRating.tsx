...
const RATINGS: { value: ReviewRating; label: string; key: string; className: string }[] = [
  { value: "hard", label: "Hard", key: "1", className: "bg-red-50 hover:bg-red-100 border-red-200 text-red-700" },
  { value: "good", label: "Good", key: "2", className: "bg-gray-50 hover:bg-gray-100 border-gray-200 text-gray-700" },
  { value: "easy", label: "Easy", key: "3", className: "bg-emerald-50 hover:bg-emerald-100 border-emerald-200 text-emerald-700" },
];
...
export function StudyRating({ card, onRate }: StudyRatingProps) {
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === " ") { e.preventDefault(); onRate("good"); }
    };
      ...
      <div className="grid grid-cols-3 gap-3">
        {RATINGS.map((r, i) => (
          ...
          >
            ...
            <span className="text-xs opacity-60 leading-snug">{getIntervalLabel(card, r.value)}</span>
